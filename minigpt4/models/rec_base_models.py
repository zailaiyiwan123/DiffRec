...
class Soft_Prompt(nn.Module):
    ...
    def user_encoder(self, users, all_users=None):
        ...
        return F.one_hot(u_.to(torch.int64), num_classes=2).float()

    def item_encoder(self, items, all_items=None):
        i_ = torch.ones_like(items).to(items.device)
        return F.one_hot(i_.to(torch.int64), num_classes=2).float()
...
class MatrixFactorization(nn.Module):
    # here we do not consider the bias term
...
class BinarizeLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training:
            x = torch.where(x == -1, torch.tensor(0, device=x.device), x)
        return x


class BinMF(nn.Module):
    def __init__(self, config, *args, **kwargs) -> None:
        super().__init__()
        self.config = config
        self.padding_index = 0
        self.user_embedding = nn.Embedding(config.user_num, config.embedding_size, padding_idx=self.padding_index)
        self.item_embedding = nn.Embedding(config.item_num, config.embedding_size, padding_idx=self.padding_index)
        self.bin = BinarizeLayer(config.embedding_size)
        print("creat MF model, user num:", config.user_num, "item num:", config.item_num)

    def user_encoder(self, users, all_users=None):
        # print("user max:", users.max(), users.min())
        return self.bin(self.user_embedding(users))

    def item_encoder(self, items, all_items=None):
        # print("items max:", items.max(), items.min())
        return self.bin(self.item_embedding(items))

    def computer(self):  # does not need to compute user representation, directly taking the embedding as user/item representations
        return None, None

    def forward(self, users, items):
        user_embedding = self.bin(self.user_embedding(users))
        item_embedding = self.bin(self.item_embedding(items))
        matching = torch.mul(user_embedding, item_embedding).sum(dim=-1)
        return matching
...
class BiasEncoder(nn.Module):
    def __init__(self, config, *args, **kwargs):
        ...
        self.config = config
        self.padding_index = 0
        self.user_bias_embedding = nn.Embedding(config.user_num, config.embedding_size, padding_idx=self.padding_index)
        self.item_bias_embedding = nn.Embedding(config.item_num, config.embedding_size, padding_idx=self.padding_index)
        nn.init.xavier_uniform_(self.user_bias_embedding.weight)
        nn.init.xavier_uniform_(self.item_bias_embedding.weight)
        print("creat Bias model, user num:", config.user_num, "item num:", config.item_num)

    def user_bias(self, users, all_users=None):
        return self.user_bias_embedding(users)

    def item_bias(self, items, all_items=None):
        return self.item_bias_embedding(items)
...
class BiasSVD(nn.Module):
    def __init__(self, config, *args, **kwargs) -> None:
        ...
        print("creat BiasSVD model, user num:", config.user_num, "item num:", config.item_num)
    ...
    def forward(self, users, items):
        ...
        item_bias = self.item_bias_embedding(items)
        matching = torch.mul(user_embedding, item_embedding).sum(dim=-1) + user_bias.sum(dim=-1) + item_bias.sum(dim=-1)
        return matching
...
class LightGCN(nn.Module):
    ...
    def predict_changed_graph(self, users, items, changed_graph):
        ...
        items = torch.from_numpy(items).long().cuda()
        with torch.no_grad():
            all_user_emb, all_item_emb = self.F_computer(self.embedding_user.weight, self.embedding_item.weight,
                                                         changed_graph)
            users_emb = all_user_emb[users]
            items_emb = all_item_emb[items]
            inner_pro = torch.mul(users_emb, items_emb).sum(dim=-1)
...
class RecEncoder_DIN(nn.Module):
    def __init__(self, args, hidden_units=[200, 80, 1]):
        ...
        self.sparse_feature_columns = [sparseFeature('uid', self.user_num, embed_dim=emb_dim),
                                       sparseFeature('iid', self.item_num, embed_dim=emb_dim)]
        ...
        self.embed_layers = nn.ModuleList(
            [nn.Embedding(feat['feat_num'], feat['embed_dim']) for i, feat in enumerate(self.sparse_feature_columns)])
