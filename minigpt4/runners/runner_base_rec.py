...
class RecRunnerBase(RunnerBase):
    ...
    def eval_epoch_pre(self, split_name, cur_epoch, skip_reload=False):
        ...
        # 新增：记录多任务损失信息
        if results and 'loss' in results:
            if isinstance(results['loss'], dict):
                # 多任务损失
                for loss_name, loss_value in results['loss'].items():
                    if isinstance(loss_value, torch.Tensor):
                        results['loss'][loss_name] = loss_value.item()
                logging.info(f"Multi-task losses: {results['loss']}")
            else:
                # 单任务损失
                logging.info(f"Single loss: {results['loss']}")
