...
if __name__ == '__main__':
    # lr_ = [1e-1,1e-2,1e-3]
    lr_ = [1e-2]  # [1e-1, 1e-2, 1e-3]  # 1e-2
    dw_ = [1e-4]  # [1e-2, 1e-4, 1e-6]
    # embedding_size_ = [32, 64, 128, 156, 512]
    embedding_size_ = [256]
    save_path = "pretrained/lightgcn/"
    f = None
    for lr in lr_:
        for wd in dw_:
            for embedding_size in embedding_size_:
                ...
                save_path += "ml_lr0.01_wd0.0001_best.pth"
                f = open("pretrained/lightgcn/ml_lr0.01_wd0.0001_best" + str(lr_[0]) + ".log", 'w+')
                # run_a_trail(train_config=train_config, log_file=f, save_mode=True, save_file=save_path,
                #             need_train=False, warm_or_cold=None)
                run_a_trail(train_config=train_config, log_file=f, save_mode=True, save_file=save_path,
