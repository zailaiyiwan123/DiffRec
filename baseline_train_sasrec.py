...
class model_hyparameters(object):
    def __init__(self):
        ...
        self.dataset = 'ml-1m'  # 'yahoo-s622-01' #'yahoo-small2' #'yahooR3-iid-001'
...
if __name__ == '__main__':
    ...
    dw_ = [1e-2, 1e-4, 1e-6]
    # embedding_size_ = [32, 64, 128, 256, 512]
    embedding_size_ = [256]
    ...
    for lr in lr_:
        for wd in dw_:
            for embedding_size in embedding_size_:
                ...
                save_path = "pretrained/sasrec/ml_lr{0}_wd{1}.pth".format(lr, wd)
                f = open("pretrained/sasrec/ml_lr{0}_wd{1}.log".format(lr, wd), 'w+')
                run_a_trail(train_config=train_config, log_file=f, save_mode=True, save_file=save_path, need_train=True,
                            warm_or_cold=None)
    if f is not None:
        f.close()
